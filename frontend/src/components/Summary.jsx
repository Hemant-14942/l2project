...
const formatBasicSummary = (text = "") => {
  return text
    ...
    .replace(/^-\s+/gm, "")
    .replace(/(\bLove\b|\bcare\b|\baffection\b|\bhappy\b|\bsafe\b|\bkind\b|\bsupportive\b|\bunderstanding\b)/gi, 
...
const formatStoryText = (text = "") => {
  return text
    .replace(/\*\*(.*?)\*\*/g, "<strong class='text-purple-300 font-semibold'>$1</strong>")
    .replace(/###\s*(.*)/g, "<h3 class='text-lg font-semibold text-white mb-3 flex items-center gap-2'>$1</h3>")
    .replace(/####\s*(.*)/g, "<h4 class='text-base font-medium text-gray-200 mb-2 flex items-center gap-2'>$1</h4>")
    .replace(/(\bZiggy\b|\bJamie\b|\bGrandma Dot\b|\bDr. Luna\b)/gi, 
      "<span class='text-purple-200 font-medium'>$1</span>")
    .replace(/(\blove\b|\bheart\b|\bcare\b|\bkindness\b|\bconnection\b)/gi, 
      "<span class='text-purple-200 font-medium'>$1</span>")
    .replace(/💖|❤️|💌|🚲|🧩|🏰|⚡️|🌈/g, "<span class='text-xl inline-block mx-1'>$&</span>")
    .replace(/\n/g, "<br />");
};
...
const formatVisualContent = (text = "") => {
  return text
    .replace(/═{10,}/g, "")
    .replace(/─{10,}/g, "")
    .replace(/#\s*(.*)/g, "<h2 class='text-xl font-semibold text-white mb-4'>$1</h2>")
    .replace(/##\s*(.*)/g, "<h3 class='text-lg font-medium text-gray-200 mb-3'>$1</h3>")
    .replace(/###\s*(.*)/g, "<h4 class='text-base font-medium text-gray-300 mb-2'>$1</h4>")
    .replace(/\*\*(.*?)\*\*/g, "<strong class='text-purple-300 font-semibold'>$1</strong>")
    .replace(/(\bLove\b|\bEmotion\b|\bBrain\b|\bChemicals\b|\bBonding\b|\bAttraction\b)/gi, 
      "<span class='text-purple-200 font-medium'>$1</span>")
    .replace(/🔴|🔵|🟢|🟡|🟣|🔶|🔷|🔸|🔹|🔺/g, "<span class='text-lg inline-block mx-1'>$&</span>")
    .replace(/```[\s\S]*?```/g, (match) => {
...
const VisualCard = ({ data }) => {
  ...
  const sections = data.split(/─{3,}|═{3,}/g).filter(section => section.trim());
...
const Summary = ({ summary, activeTab, setActiveTab, resetForm }) => {
  const getTabIcon = (tab) => {
    switch(tab) {
      case 'basic': return <FileText size={16} />;
  ...
  return (
    <div className="w-full max-w-5xl mx-auto mt-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <button
          onClick={resetForm}
          className="flex items-center gap-2 px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-full transition-all duration-300 font-medium"
        >
          <ArrowLeft size={16} />
