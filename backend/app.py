...
with tab1:
    ...
    if upload_type == "📄 PDF Document":
        ...
        if uploaded_file:
            with st.spinner("🔄 Extracting text from PDF..."):
                ...
                content_title = os.path.splitext(uploaded_file.name)[0]
        ...
        if youtube_url and st.button("🎬 Process Video"):
            ...
            with st.spinner("🎥 Extracting transcript from video..."):
                ...
                title, text = components['file_processor'].extract_youtube_transcript(youtube_url)
            ...
            if text:
                ...
                st.success(f"✅ Video '{title}' processed!")

                st.text_area("Transcript Preview:", text[:500] + "...", height=150)
        ...
        if audio_file:
            ...
            if text:
                st.session_state.content_title = "Voice Input"
    ...
    # Save content
    if 'content_text' in st.session_state and 'content_title' in st.session_state:
        if st.button("💾 Save Content for Learning"):
            filepath = components['file_processor'].save_content(
                st.session_state.content_text,
                st.session_state.content_title,
                upload_type
            )
            if filepath:
                st.session_state.current_content = st.session_state.content_text
                st.session_state.current_title = st.session_state.content_title
                st.session_state.upload_type = upload_type
                st.session_state.pop('summaries', None)
                st.session_state.pop('flashcards', None)

                ai_coach.add_to_knowledge_base(
                    st.session_state.content_text[:1000],
                    st.session_state.content_title,
                    upload_type
                )
